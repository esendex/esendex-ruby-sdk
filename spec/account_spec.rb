...
describe Account do
  ...
  describe "#request_message_status" do
    ...
    before(:each) do
      api_connection.stub(:get) { mock('Response', :body => send_response_xml) }
    end

    subject { account.request_message_status(message_id) }

    it "should post to the message dispatcher resource" do
      api_connection.should_receive(:get).with("/v1.0/messageheaders/#{message_id}")
      subject
    end
    ...
    it "should return phone number of recipient" do
      subject.phonenumber.should eq phonenumber
    end
  end
